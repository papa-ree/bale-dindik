<header class="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md shadow-sm select-none" data-aos="fade-down"
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-20">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="font-bold text-primary text-lg lg:text-xl leading-tight">Dinas Pendidikan</div>
                    <div class="text-gray-600">Kabupaten Ponorogo</div>
                </div>
            </div>
            ...
            <nav class="hidden lg:flex items-center gap-8">
                ...
                @foreach ($this->availableNavs as $key => $nav)
                    @if ($nav->children->isNotEmpty())
                        <div
                            ...
                            <button id="bale-dindik-{{$nav->slug}}" type="button"
                                class="hs-dropdown-toggle w-full flex items-center  text-gray-700 hover:text-primary font-medium transition-colors duration-300 relative group focus:outline-hidden hs-scrollspy-active:bg-gray-100 dark:hs-scrollspy-active:bg-neutral-700"
                            ...
                            <div class="hs-dropdown-menu transition-[opacity,margin] duration-[0.1ms] md:duration-150 hs-dropdown-open:opacity-100 opacity-0 relative w-full md:w-52 hidden z-10 top-full ps-7 md:ps-0 md:bg-white/95 md:rounded-lg md:shadow-md before:absolute before:-top-4 before:start-0 before:w-full before:h-5 md:after:hidden after:absolute after:top-1 after:start-4.5 after:w-0.5 after:h-[calc(100%-4px)] after:bg-gray-100 dark:md:bg-neutral-900 dark:after:bg-neutral-700"
                                role="menu" aria-orientation="vertical" aria-labelledby="bale-dindik-{{$nav->slug}}">
                                <div class="py-1 md:px-1 space-y-0.5">

                                    @foreach ($nav->children as $navItem)
                                        <div>
                                            <a class="flex items-center p-2 text-gray-700 hover:text-primary font-medium transition-color duration-300 ease-in-out group rounded-lg md:px-3 focus:outline-none dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:bg-gray-700 dark:focus:text-gray-300"
                                                @if ($navItem->url_mode) href="{{ $navItem->url }}"
                                                target="{{ $navItem->target ?? '_self' }}" @else
                                                    href="{{ route('bale.view-page', $navItem->page_slug ?? '404') }}"
                    ...
                    @else
                        <a class="text-gray-700 hover:text-primary font-medium transition-colors duration-300 relative group"
            ...
            <button type="button" disabled
                class="hidden lg:inline-flex bg-primary text-white px-6 py-2.5 rounded-lg font-semibold transition-all duration-300">
                Website Resmi
            </button>

            {{-- Mobile Menu Button --}}
            <button @click="mobileMenuOpen = !mobileMenuOpen" class="lg:hidden p-2 text-primary">
